        ...
        <div class="col text-md-left text-center">
          @if (json_decode($order->shipping_address))
            <address>
              ...
              @endif
              {{ isset(json_decode($order->shipping_address)->postal_code) ? json_decode($order->shipping_address)->postal_code : '' }}
    ...
    <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 540px;">
      <div class="modal-content p-2rem">
        <div class="modal-body text-center">
          ...
          <p class="fs-16 fw-700 mb-3 mt-3">{{ translate('Are you sure you want to change the payment status?') }}</p>
          <button type="button" class="btn btn-light rounded-2 fs-13 fw-700 w-150px mt-2"
                  data-dismiss="modal">{{ translate('Cancel') }}</button>
          <button type="button" onclick="update_payment_status()"
                  class="btn btn-success rounded-2 fs-13 fw-700 w-150px mt-2">{{ translate('Confirm') }}</button>
        </div>
      </div>
    </div>
  </div>
