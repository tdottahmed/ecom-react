...
<div class="card">
    ...
    <form class="p-4" action="{{ route('custom-pages.update', $page->id) }}" method="POST"
        ...
        <div class="card-body px-0">
            ...
            <div class="form-group row">
                <label class="col-sm-2 col-from-label">{{ translate('Attach Products') }}</label>
                <div class="col-sm-10">
                    <select class="form-control aiz-selectpicker" name="product_ids[]" multiple data-live-search="true"
                            data-actions-box="true">
                        @foreach($products as $product)
                        <option value="{{ $product->id }}" @if(in_array($product->id, $selectedProducts ?? [])) selected
                            @endif>
                            {{ $product->name }} ({{ $product->slug }})
                        </option>
                        @endforeach
                    </select>
                    <small class="form-text text-muted">{{ translate('Select products to show on this page') }}</small>
                </div>
            </div>
        </div>
            ...
            <div class="form-group row mb-0">
                ...
                <div class="col-sm-10">
                    <label class="aiz-switch aiz-switch-success mb-0">
                        <input type="checkbox" name="is_active" @if($page->is_active) checked @endif>
                        <span></span>
                    </label>
                </div>
            </div>
