<section class="py-10">
    <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="mb-8">
            <img
                src="{{ $detailedProduct->photos ? uploaded_asset(explode(',', $detailedProduct->photos)[0]) : asset('images/placeholder.jpg') }}"
        ...
        <!-- Product Info Box -->
        <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg text-center space-y-5" id="product-info-box">
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">{{ $detailedProduct->name }}</h1>
            ...
            <!-- Price -->
            <div class="space-x-3">
                <span class="text-3xl font-bold text-indigo-600" id="variant-price">
                    ৳{{ number_format($detailedProduct->unit_price, 2) }}
                </span>
                @if($detailedProduct->discount > 0)
                    @php
                        $originalPrice = $detailedProduct->discount_type == 'percent'
            ...
            @if(isset($detailedProduct->stocks) && count($detailedProduct->stocks) > 0)
                <div>
                    <h3 class="text-sm font-semibold mb-2">Choose Variation:</h3>
                    <div class="flex flex-wrap justify-center gap-2">
                        @foreach($detailedProduct->stocks as $stock)
                            <button
                                class="px-3 py-1 border rounded text-sm
                                       {{ $stock->qty == 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'hover:bg-indigo-100 text-gray-800' }}
                                       {{ $loop->first ? 'bg-indigo-600 text-white' : '' }} variation-option"
                                data-variant-id="{{ $stock->id }}"
            ...
            <div class="flex justify-center items-center space-x-2 mt-3">
                <button id="decrease-qty" class="px-3 py-1 border rounded bg-gray-100 hover:bg-gray-200">-</button>
                <input type="number" id="product-quantity"
                       class="w-16 text-center border rounded" value="1" min="1"
                       max="{{ $detailedProduct->current_stock }}">
                <button id="increase-qty" class="px-3 py-1 border rounded bg-gray-100 hover:bg-gray-200">+</button>
            </div>

            <!-- Order Now -->
            <div class="mt-6 flex justify-center">
                <button id="buy-now-btn"
                        class="flex items-center justify-center px-8 py-4 bg-indigo-600 text-white font-bold text-lg rounded-full shadow-lg">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        gsap.set(productimage, {transformOrigin: "40% 0%"});
        ...
        gsap.from("#main-product-image", {opacity: 0, y: 50, duration: 1});
        gsap.from("#product-info-box", {opacity: 0, y: 50, duration: 1, delay: 0.3, ease: "power2.out"});
    });
</script>
